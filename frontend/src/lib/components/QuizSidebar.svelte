<script lang="ts">
	import type { QuizQuestion } from '$lib/api/entities';
	...
	function cellClasses(question: QuizQuestion, index: number) {
		if (index === currentIndex) {
			return 'bg-indigo-400 text-white shadow-lg shadow-indigo-900/30';
		}

		if (mode === 'review') {
			if (question.isCorrect === true) return 'bg-emerald-400/15 text-emerald-300';
			if (question.isCorrect === false) return 'bg-rose-400/15 text-rose-300';
			...
			<div class="grid grid-cols-4 gap-2.5">
				{#each questions as question, i (question.id)}
					...
					>
						{question.id}
						{#if i !== currentIndex}
							{#if mode === 'taking' && question.selected !== null}
								<Check
									class="absolute -right-1 -top-1 h-3.5 w-3.5 rounded-full bg-emerald-400 p-0.5 text-[#0C1550]"
								/>
							{:else if mode === 'review' && question.isCorrect === true}
								<Check
									class="absolute -right-1 -top-1 h-3.5 w-3.5 rounded-full bg-emerald-400 p-0.5 text-[#0C1550]"
								/>
							{:else if mode === 'review' && question.isCorrect === false}
								<X class="absolute -right-1 -top-1 h-3.5 w-3.5 rounded-full bg-rose-400 p-0.5 text-[#0C1550]" />
							{/if}
						{/if}
					</button>
				{/each}
